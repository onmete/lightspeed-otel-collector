...
.PHONY: test
test: ## Run unit tests.
	cd agenticdataexporter && go test -v ./...
	cd postgresexporter && go test -v ./...
	cd extension/postgresadmin && go test -v ./...
	cd extension/httpsmetrics && go test -v ./...

.PHONY: test-e2e
test-e2e: ## Run e2e tests (requires OTEL_COLLECTOR_IMAGE and cluster access via KUBECONFIG).
	@test -n "$(OTEL_COLLECTOR_IMAGE)" || (echo "ERROR: OTEL_COLLECTOR_IMAGE is required"; exit 1)
	cd test/e2e && go test -tags=e2e -count=1 -v -timeout 15m ./...

.PHONY: lint
lint: ## Run golangci-lint.
	cd agenticdataexporter && golangci-lint run ./...
	cd postgresexporter && golangci-lint run ./...
	cd extension/postgresadmin && golangci-lint run ./...
	cd extension/httpsmetrics && golangci-lint run ./...

.PHONY: fmt
fmt: ## Run go fmt.
	cd agenticdataexporter && go fmt ./...
	cd postgresexporter && go fmt ./...
	cd extension/postgresadmin && go fmt ./...
	cd extension/httpsmetrics && go fmt ./...

.PHONY: vet
vet: ## Run go vet.
	cd agenticdataexporter && go vet ./...
	cd postgresexporter && go vet ./...
	cd extension/postgresadmin && go vet ./...
	cd extension/httpsmetrics && go vet ./...
